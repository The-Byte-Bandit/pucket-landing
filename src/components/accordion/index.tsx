...
import { fadeIn, fadeUp, staggerContainer } from "../motion";
...
export default function Accordion({ items }: AccordionProps) {
    return (
        <motion.div className="mx-auto max-w-4xl" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={staggerContainer}>
            {items.map((it) => (
                <motion.details key={it.id} variants={fadeIn} className="group rounded-2xl border border-white/10 bg-white/[0.03] mb-3 overflow-hidden">
